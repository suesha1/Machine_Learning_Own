...
data=pd.read_csv("df_noise(0.8).csv")
...
async def predict(item_id: int, item: Item = Body(..., embed=True)):
     ...
     model=iso.iForest(X_train, ntrees=110, sample_size=135, ExtensionLevel=6)
     print(model)
     lis=feature_normalize(item)
     print(lis)
     temp = np.asarray(lis)
     X_test  = np.reshape(temp ,(1,  temp.size))
     print(X_test.shape)
     res = model.compute_paths(X_test)
     print(res)
     val=np.argsort(res)
     if res[val][0]<0.5:
